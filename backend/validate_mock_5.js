...
const mock5Path = path.join(__dirname, '../frontend/src/data/mockTests/iat_full_05.json');
const catalogPath = path.join(__dirname, 'data/learningCatalog.json');

console.log('='.repeat(60));
console.log('MOCK #5 MIGRATION VALIDATION SCRIPT');
console.log('='.repeat(60));

// 1. Load current JSON
let currentData;
try {
  currentData = JSON.parse(fs.readFileSync(mock5Path, 'utf8'));
} catch (err) {
  console.error('❌ Failed to parse migrated iat_full_05.json:', err.message);
  process.exit(1);
}
...
if (passed) {
  console.log('✅ ALL MOCK #5 MIGRATION INTEGRITY CHECKS PASSED SUCCESSFULLY!');
  ...
  console.error(`❌ MOCK #5 MIGRATION VALIDATION FAILED with ${errors.length} error(s):`);
