...
const mock3Path = path.join(__dirname, '../frontend/src/data/mockTests/iat_full_03.json');
const catalogPath = path.join(__dirname, 'data/learningCatalog.json');

console.log('='.repeat(60));
console.log('MOCK #3 MIGRATION VALIDATION SCRIPT');
console.log('='.repeat(60));

// 1. Load current JSON
let currentData;
try {
  currentData = JSON.parse(fs.readFileSync(mock3Path, 'utf8'));
} catch (err) {
  console.error('❌ Failed to parse iat_full_03.json:', err.message);
  process.exit(1);
}
...
if (passed) {
  console.log('✅ ALL MOCK #3 INTEGRITY CHECKS PASSED SUCCESSFULLY!');
  ...
  console.error(`❌ MOCK #3 VALIDATION FAILED with ${errors.length} error(s):`);
