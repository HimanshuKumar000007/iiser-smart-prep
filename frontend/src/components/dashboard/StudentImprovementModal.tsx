...
export function StudentImprovementModal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-2.5 sm:p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150"
      onClick={(e) => {
        if (e.target === e.currentTarget) handleDismiss();
      }}
    >
      {/* Vercel Dialog Frame - Mobile Responsive with Fixed Header & Footer */}
      <div
        className={cn(
          'relative w-full max-w-2xl max-h-[88dvh] sm:max-h-[85vh] flex flex-col rounded-2xl border transition-all overflow-hidden shadow-2xl',
          isLight
      ...
      >
        {/* Top Minimal Bar - Fixed at top, never cut off */}
        <div className={cn(
          'flex items-center justify-between px-4 sm:px-6 py-3 sm:py-3.5 border-b shrink-0 z-10',
          isLight ? 'border-neutral-100 bg-neutral-50/80' : 'border-[#1a1a1a] bg-[#0f0f0f]/90'
        )}>
        ...
        {isSubmitted ? (
          ...
          <div className="p-8 sm:p-12 text-center space-y-3 flex-1 flex flex-col items-center justify-center">
            <div className={cn(
          ...
          /* Flex Column Form with Scrollable Body & Pinned Footer */
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-5 overscroll-contain">
              ...
              {/* 6 Clean, Non-Truncated Cards (Sleek on Mobile) */}
                ...
                {IMPROVEMENT_OPTIONS.map(opt => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'flex items-start gap-2.5 sm:gap-3 p-2.5 sm:p-3.5 rounded-xl border text-left transition-all duration-150 cursor-pointer group select-none',
                        isSelected
                          ? isLight
                    ...
                    >
                      <div
                        className={cn(
                          'w-7 h-7 sm:w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-colors mt-0.5',
                          isSelected
                            ? 'bg-white text-black'
                      ...
                      <div className="flex-1 min-w-0 pr-1">
                        ...
                        <div className={cn(
                          'text-[10px] sm:text-[11px] leading-tight sm:leading-relaxed mt-0.5',
              ...
              <div className="space-y-2 pt-1">
                  ...
                  {SUGGESTED_CHIPS.map(chip => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          'text-[11px] sm:text-xs px-2 sm:px-2.5 py-1 rounded-md border transition-all duration-150 cursor-pointer font-medium select-none',
                          isActive
                            ? isLight
                ...
                <textarea
                  ...
                  rows={2}
                  className={cn(
                    'w-full text-xs rounded-xl p-2.5 sm:p-3 border outline-none resize-none transition-all placeholder:text-neutral-500',
                    isLight
            ...
            {/* Fixed Sticky Footer - Always visible on mobile, never pushed off-screen */}
            <div className={cn(
              'flex items-center justify-between px-4 sm:px-6 py-3 border-t shrink-0 z-10',
              isLight ? 'border-neutral-100 bg-white' : 'border-[#1a1a1a] bg-[#0c0c0c]'
            )}>
              <button
                type="button"
                onClick={handleDismiss}
